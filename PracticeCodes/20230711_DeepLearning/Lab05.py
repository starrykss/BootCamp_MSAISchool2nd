# Lab 05. PySide 6 실습 : 시그널과 슬롯의 연결 응용

"""
1. 사용자로부터 나이, 성별, 국가 입력 받기
2. 보기 버튼 누르면 입력 정보를 보여준다.
3. 입력 정보창에서는 (나이, 성별, 국가)를 입력 받은 정보를 출력한다.
- 추가적으로 저장 버튼, 닫기 버튼 불러오기 버튼 추가
4. 저장된 정보 창에서는 리스트 박스를 이용하여 (중복 방지를 위한 ID 추가, 여기서는 Time 함수를 이용) DB 처럼 Auto Number 없이 저장된 정보를 읽어 오기
- ID / 나이 / 성별 / 국가
"""
